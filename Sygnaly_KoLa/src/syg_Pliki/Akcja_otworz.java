...
public class Akcja_otworz {
	...
	private void ustawAliasy(XStream _xstream) {
		...
		if (this.sygnal.getRodzaj() == RodzajSygnalu.CIAGLY) {
			...
			_xstream.omitField(Sygnal.class, "punktyNaWYkresie");
	...
	private void odczytajZPliku() {
		...
		if (wyborPlikuDoOdczytu.showOpenDialog(source) == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				Sygnal syg = (Sygnal) xstream.fromXML(new FileInputStream(plikDoOdczytu));
				// Sygnal syg = (Sygnal) xstream.fromXML(plikDoOdczytu);

				przygotujObiektPoDeserializacji(syg);

				JOptionPane.showMessageDialog(null, "Operacja odczytu zakończona powodzeniem.",
			...
			// }
				...
				JOptionPane.showMessageDialog(null,
						"Nieznany błąd.\nOpis błędu" + _err.getMessage(), "Błąd.",
						JOptionPane.ERROR_MESSAGE);
			}
		}
	}
